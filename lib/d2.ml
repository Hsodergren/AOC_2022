...
let score res move =
  let score_res = function Win -> 6 | Draw -> 3 | Lose -> 0 in
  let score_move = function Rock -> 1 | Paper -> 2 | Scissor -> 3 in
...
let part2 seq =
  let sum =
    run seq parse_part2 (fun a b ->
        match b with
        | Draw -> a
        | Win -> (
            match a with Paper -> Scissor | Rock -> Paper | Scissor -> Rock)
        | Lose -> (
            match a with Paper -> Rock | Rock -> Scissor | Scissor -> Paper))
  in
  Fmt.pr "%d\n" sum
